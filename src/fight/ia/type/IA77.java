...
public class IA77 extends AbstractNeedSpell {
    ...
    public IA77(Fight fight, Fighter fighter, byte count) {
        super(fight, fighter, count,"IA77");
    }
    ...
    public void apply() {
        if (!this.stop && this.fighter.canPlay() && this.count > 0) {
            ...
            if (this.fighter.getCurPa(this.fight) > 0 && longestEnnemy != null && nearestEnnemy == null ) {
                int value = Function.getInstance().attackIfPossible(this.fight, this.fighter, this.highests);
                if (value != 0) {
                    time = value;
                    action = true;
                }
            } else if (this.fighter.getCurPa(this.fight) > 0 && nearestEnnemy != null  ) {
