...
//	public static int applySpecificCases(int finalDommage, Fighter target, Fighter caster, Fight fight, int elementId) {
//		for (int id : Constant.ON_HIT_BUFFS) {
//			for (Effect buff : target.getBuffsByEffectID(id)) {
//				switch (id) {
//					case 79://chance éca Ca faudrait le géré dans le calcul des dommages aussi (meme fonction)
//						try {
...
//						}
//						break;
//					case 107://renvoie Dom, ca faudrait le géré dans le calcul des dégats ou avant en vrai non ?
...
//		this.dammageDone = 0;
//
//		this.elem = getElemSwitchEffect(effectID);
//
//		// LES EFFETS QUI N'ONT PAS BESOIN DE TARGET
//		if(ArrayUtils.contains(EffectConstant.NONEEDTARGET_EFFECT,effectID)){
//			switch (effectID){
...
//				if (ArrayUtils.contains(EffectConstant.IS_DIRECTDAMMAGE_EFFECT, effectID) && turns == 0) {
//					if (caster.isHide())
//						caster.unHide(spell);
//
...
//							applyEffect_DammageElem(target, fight, isCaC);
//							break;
//						case 101://Retrait PA
...
//				finalDommage = applySpecificCases(finalDommage, target, caster, fight, Constant.ELEMENT_NULL);//S'il y a des buffs sp�ciaux
...
//					val = 0;
//
//				val = applySpecificCases(val, target, caster, fight, Constant.ELEMENT_NULL);//S'il y a des buffs sp�ciaux
//
...
//		int elem = getElemSwitchEffect(this.effectID);
//		if (turns == 0) {
//				int dmg = Formulas.getRandomJet(jet, caster, target);
//				int finalDommage = calculFinalDommage(fight, caster, target, elem, dmg, false, isCaC, spell);
//				finalDommage = applySpecificCases(finalDommage, target, caster, fight, elem);//S'il y a des buffs sp�ciaux
//
//				//target = getRealTargetSwitchBuff(target);
//				if (finalDommage > target.getPdv())
//		 			finalDommage = target.getPdv();//Target va mourrir
//				target.removePdv(caster, finalDommage);
//
//				sendDammageClient(fight,target.getId(), caster.getId(),finalDommage,this.effectID);
//				// Vol de vie neutre
//				int heal = calculLifeSteal(caster,finalDommage);
//				caster.addPdv(caster, heal);
//
//				sendHealClient(fight,target.getId(), caster.getId(),heal);
//
//
//
//				this.dammageDone = finalDommage;
...
//	}
//
//	private int getElemSwitchEffect(int EffectID){
...
//	}
//
//	private int calculLifeSteal(Fighter caster,int dammage){
//		int heal = (int) Math.floor ((dammage) / 2);
//		if ((caster.getPdv() + heal) > caster.getPdvMax())
//			heal = caster.getPdvMax() - caster.getPdv();
//
//		return heal;
//	}
//
//	private void sendDammageClient(Fight fight, int targetId, int casterId, int value, int effectID){
//		int dammage = -(Math.abs(value));
//
//		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, casterId
//				+ "", targetId + "," + dammage, effectID);
//	}
//
//	private void sendHealClient(Fight fight, int targetId, int casterId, int value){
//		int heal = Math.abs(value);
//		SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 108, targetId
//				+ "", casterId + "," + heal, 108);
//	}
//
//	private void applyEffect_DammageElem(Fighter target, Fight fight,
//								boolean isCaC)//dmg eau
//	{
//
//		if (turns == 0) {
//				int dmg = Formulas.getRandomJet(jet, caster, target);
//				int finalDommage = Formulas.calculFinalDommage(fight, caster, target, this.elem, dmg, false, isCaC, spell);
//				finalDommage = applySpecificCases(finalDommage, target, caster, fight, this.elem);//S'il y a des buffs sp�ciaux
//
//				if (finalDommage > target.getPdv())
//					finalDommage = target.getPdv();//Target va mourrir
//				target.removePdv(caster, finalDommage);
...
//				}*/
//				/*finalDommage = -(finalDommage);
//				SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, 100, caster.getId()+ "", target.getId() + "," + finalDommage, this.effectID);*/
//				sendDammageClient(fight,target.getId(), caster.getId(),finalDommage,this.effectID);
//
//			this.dammageDone = finalDommage;
//
...
//
//			finalDommage = applySpecificCases(finalDommage, caster, caster, fight, Constant.ELEMENT_NULL);//S'il y a des buffs sp�ciaux
//			if (finalDommage > caster.getPdv())
//				finalDommage = caster.getPdv();//Caster va mourrir
//			caster.removePdv(caster, finalDommage);
//			/*finalDommage = -(finalDommage);
...
//	}
//
//	private void applyEffect_Buff(Fighter target, Fight fight)//buff
//	{
//		int val = Formulas.getRandomJet(jet, caster);
//		if (val == -1) {
//			GameServer.a("Valeur jet negatif " + spell);
...
//		caster.addBuff(effectID, fixvalue, turns, 1, false, spell, args, caster, true);
...
//				if (val < 0)
//					val = 0;
//				val = applySpecificCases(val, target, caster, fight, Constant.ELEMENT_NULL);
//				if (val > target.getPdv())
//					val = target.getPdv();
//				target.removePdv(caster, val);
//				int cura = val;
...
//
//			int finalDommage = applySpecificCases(dgt, target, caster, fight, Constant.ELEMENT_NEUTRE);//S'il y a des buffs sp�ciaux
...
//		target.addBuff(effectID, info, turns, 1, true, spell, args, caster, true);
//	}
//
//	// TODO a reprendre si c'est utile
