...
public class Mount {
    ...
    public final static Updatable updatable = new Updatable(360000) {
        @Override
        public void update() {
            if(this.verify()) {
                for (Mount mount : World.world.getMounts().values()) {
                    if (mount.getFatigue() <= 0) continue;
                    mount.setFatigue(mount.getFatigue() - 1);
                ...
                if(mountArg.getSex() !=	mount.getSex() && mountArg.isFecund() != 0 && mount.isFecund() != 0 && mountArg.getOwner() == mount.getOwner()) {
					...
					if(mountArg.getReproduction() < 20 && mount.getReproduction() < 20 && !mountArg.isCastrated() && !mount.isCastrated()) {
						...
						if(mountArg.getSex() == 1) {
							...
							mountArg.resEndurance(2500);
							mount.resAmor(2500);
							mount.resEndurance(2500);
	...
	public synchronized void checkBaby(Player player) {
        ...
        if(time2 < actualHours && actualHours < time + 24 * 7) {
			...
			int offspring = 2 + (reproductrice ? 2 : 0), value = Formulas.getRandomValue(0, 16), max = 4 + (reproductrice ? 2 : 0);

			if(value >= 5 && value <= 10)
				offspring = (reproductrice ? 4 : 3);
			else if(value < 1)
				offspring = max;
	...
	public void setCapacitys(int capacity) {
		this.capacitys.add(capacity);
		//return capacitys;
	}
	...
	private void aumFatige() {
		this.fatigue += 9;
		if(this.fatigue > 239) this.fatigue = 239;
        Database.getStatics().getMountData().update(this);
	}

	private void aumEndurance(int endurance) {
		this.endurance += (endurance / 10) * this.getBonusFatigue() ;// * Generation.getLearningRate(Constant.getGeneration(this.color));
	...
	private void aumMaturity(int Resist) {
		if(this.maturity < this.getMaxMaturity()) {
			this.maturity += (Resist / 10) * this.getBonusFatigue();
			if(this.capacitys.contains(7))
				this.maturity += Resist / 5;
			if(this.size < 100) {
	...
	private void aumAmor(int amour) {
		this.amour += (amour / 10) * this.getBonusFatigue();
		if(this.capacitys.contains(6)) this.amour += amour / 50;
		if(this.amour > 10000) this.amour = 10000;
	}

	private void aumState(int state) {
		this.state += (state / 10) * this.getBonusFatigue();
		if(this.state > 10000) this.state = 10000;
	}

	public void aumEnergy(int energy) {
		this.energy += (energy / 50) * this.getBonusFatigue();
		if(this.capacitys.contains(1)) this.energy += energy / 50;
	...
	private double getBonusFatigue() {
		if(this.fatigue > 0 && this.fatigue <= 160)
			return 1.5;
